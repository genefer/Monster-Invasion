			...
			switch (currentScreen)
			{
                ...
                case GameScreen.GameOverScreen:
                    ...
                    if (((tickLast - delayStart) / delayBlink) % 3 != 1) {
                        pos.Y = 420;
                        TextHandler.DrawTextCentered("PRESS THE BACK BUTTON", pos);
                        pos.Y += 30;
                        TextHandler.DrawTextCentered("TO RETURN TO THE MAIN MENU", pos);
                    }
					break;
                ...
                case GameScreen.CreditsScreen:
                    ...
                    TextHandler.DrawTextCentered("THIS GAME IS OPEN-SOURCE FREEWARE!", new Vector2(horizCenter, yPos)); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("FIND THE SOURCE CODE AT CODEPROJECT.COM", new Vector2(horizCenter, yPos)); yPos += 3 * lineSpacing / 2;

                    TextHandler.DrawTextCentered("FEEL FREE TO EMAIL US WITH QUESTIONS OR", new Vector2(horizCenter, yPos)); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("ABOUT OUR SOFTWARE DEVELOPMENT SERVICES:", new Vector2(horizCenter, yPos)); yPos += 3 * lineSpacing / 2;

                    TextHandler.DrawTextCentered("WINDOWS - ORIGINAL DIRECTX VERSION, 2002", new Vector2(horizCenter, yPos), 1.0f, Color.Gray); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("MAURICIORITTER@HOTMAIL.COM", new Vector2(horizCenter, yPos), 1.0f, Color.Gray); yPos += 4 * lineSpacing / 3;
                    
                    TextHandler.DrawTextCentered("WINDOWS - MANAGED DIRECTX VERSION, 2003", new Vector2(horizCenter, yPos), 1.0f, Color.Gray); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("STEVE@YSGARD.COM", new Vector2(horizCenter, yPos), 1.0f, Color.Gray); yPos += 4 * lineSpacing / 3;
                    
                    TextHandler.DrawTextCentered("WINDOWS PHONE - XNA VERSION, 2011", new Vector2(horizCenter, yPos)); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("SUPPORT@IMPROVISOFT.COM", new Vector2(horizCenter, yPos)); yPos += lineSpacing;
                    TextHandler.DrawTextCentered("WWW.IMPROVISOFT.COM", new Vector2(horizCenter, yPos)); yPos += lineSpacing;
                ...
                case GameScreen.OptionsScreen:
                    ...
                    if (!GameInProgress) {
                        ...
                        TextHandler.DrawTextCentered("SCOREBOARD POSITION: " + scoreboardPos, pos, 1.5f, rowColor); pos.Y += optionsMenuItemDeltaY;
                    }

                    rowColor = (selectedEntry == ++iEntry)? Color.White : dimRowColor;
                    TextHandler.DrawTextCentered("GAME MUSIC: " + gameMusicEnabled, pos, 1.5f, rowColor); pos.Y += optionsMenuItemDeltaY;

                    rowColor = (selectedEntry == ++iEntry)? Color.White : dimRowColor;
                    TextHandler.DrawTextCentered("DIFFICULTY LEVEL: " + difficultyText, pos, 1.5f, rowColor); pos.Y += optionsMenuItemDeltaY;

                    rowColor = (selectedEntry == ++iEntry)? Color.White : dimRowColor;
                    TextHandler.DrawTextCentered("AUTO-SELECT WEAPON: " + autoSelectWeaponText, pos, 1.5f, rowColor); pos.Y = options2ndSubmenuY;


                    rowColor = (selectedEntry == ++iEntry)? Color.White : dimRowColor;
                    TextHandler.DrawTextCentered("RATE AND REVIEW INVASION", pos, 1.5f, rowColor); pos.Y += optionsMenuItemDeltaY;

                    rowColor = (selectedEntry == ++iEntry)? Color.White : dimRowColor;
                    TextHandler.DrawTextCentered("FIND OUR OTHER GAMES!", pos, 1.5f, rowColor); pos.Y += optionsMenuItemDeltaY;

                    if (((tickLast - delayStart) / delayBlink) % 3 != 1) {
                        pos.Y = 420;
                        TextHandler.DrawTextCentered("PRESS THE BACK BUTTON", pos, 1.0f, Color.White);
                        pos.Y += 30;
                        if (GameInProgress)
                            TextHandler.DrawTextCentered("TO RESUME YOUR GAME", pos);
                        else
                            TextHandler.DrawTextCentered("TO RETURN TO THE MAIN MENU", pos);
                    }
